...
export function APAgingList() {
  ...
  const handleExport = async (format: "pdf" | "xlsx") => {
    setExporting(format);
    try {
      const blob = await api.exportReport("ap_aging", { format, as_of: asOf });
      downloadBlob(blob, `ap-aging-${asOf}.${format}`);
      showToast(`AP aging exported as ${format.toUpperCase()}.`);
    } catch (err) {
    ...
    <div className="listtab listtab--accurate">
      <div className="listtab__head">
        <div className="listtab__title">
          <span>AP Aging</span>
          <small>Payables by age bucket as of the selected date.</small>
        </div>
      </div>
