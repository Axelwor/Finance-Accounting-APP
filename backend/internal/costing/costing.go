...
func PostGRN(ctx context.Context, tx pgx.Tx, tenantID, itemID int64, qty float64, unitCostCents int64, costingMethod string) error {
	if qty <= 0 {
	...
	case MethodMovingAverage:
		...
		// taken by the UPSERT.
		_, err := tx.Exec(ctx, `
			INSERT INTO stock_balances (tenant_id, item_id, qty_on_hand, avg_unit_cost_cents, updated_at)
			VALUES ($1, $2, $3, $4, now())
			ON CONFLICT (tenant_id, item_id) DO UPDATE
			SET avg_unit_cost_cents = CASE
			        WHEN stock_balances.qty_on_hand + $3 = 0 THEN stock_balances.avg_unit_cost_cents
			        ELSE (
			    ...
			    updated_at = now()
		`, tenantID, itemID, qty, unitCostCents)
		if err != nil {
...
func ResolveCOGS(ctx context.Context, tx pgx.Tx, tenantID, itemID int64, qty float64, costingMethod string) (totalCOGSCents int64, err error) {
	if qty <= 0 {
	...
	qoh, avgCost, err := lockBalance(ctx, tx, tenantID, itemID)
	if err != nil {
		return 0, err
	}
	if qoh < qty {
	...
	switch method {
	case MethodFIFO:
		return consumeFIFO(ctx, tx, tenantID, itemID, qty, qoh, float64(avgCost))
	case MethodMovingAverage:
...
// still succeeds.
func consumeFIFO(ctx context.Context, tx pgx.Tx, tenantID, itemID int64, qty, qoh, avgCost float64) (int64, error) {
	rows, err := tx.Query(ctx, `
		SELECT id, qty_remaining, unit_cost_cents
		FROM inventory_cost_layers
		WHERE tenant_id = $1 AND item_id = $2 AND closed_at IS NULL
	...
	for remaining > 0 && rows.Next() {
		...
		if newRemaining <= 0 {
			...
			if _, err := tx.Exec(ctx, `
				...
				SET qty_remaining = $3
				WHERE tenant_id = $1 AND id = $2
			`, tenantID, layerID, newRemaining); err != nil {
...
// costingMethod should be read from items.costing_method; if empty the method
// is read from the items table.
func ReverseCOGS(ctx context.Context, tx pgx.Tx, tenantID, itemID int64, qty float64, unitCostCents int64, costingMethod string) error {
	if qty <= 0 {
...
// zeros.
func GetStockBalance(ctx context.Context, tx pgx.Tx, tenantID, itemID int64) (qtyOnHand float64, avgCostCents int64, err error) {
	return lockBalance(ctx, tx, tenantID, itemID)
}
...
func lockBalance(ctx context.Context, tx pgx.Tx, tenantID, itemID int64) (float64, int64, error) {
	...
	err := tx.QueryRow(ctx, `
		...
		WHERE tenant_id = $1 AND item_id = $2
		FOR UPDATE
	`, tenantID, itemID).Scan(&qoh, &avgCost)
	if err != nil {
		if isNoRows(err) {
			return 0, 0, nil
		}
